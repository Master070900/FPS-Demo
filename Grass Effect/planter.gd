...
const MeshFactory = preload("res://Grass Effect/mesh_factory.gd")

export(float) var span = 5 setget set_span
...
export(Vector2) var sway_yaw = Vector2(0, 10) setget set_sway_yaw
export(Vector2) var sway_pitch = Vector2(0, 10) setget set_sway_pitch
...
func rebuild():
	...
	for index in range(0, multimesh.instance_count):
		var pos = Vector3(rand_range(-span, span), 0, rand_range(-span, span))
		var basis = Basis(Vector3.UP, deg2rad(rand_range(0, 360)))
