...
const h_weight:int = 20 # Stiffness of the gun while moving camera horizontally
const v_weight:int = 30 # Stiffness of the gun while moving camera vertically
const weight:float = 1.5 # How much the player slows down while holding this gun
const reload_time:float = 2.1 # How long it takes to reload (Independent of reload animation)
const hip_offset:int = 5 # The maximum angle the raycast can rotate when not ads
...
var max_ammo:int = 30 # Maximum ammo the gun has
var ammo:int = max_ammo # The ammo left in mag
var shoot_cooldown:float = 0.15 # How long in seconds it takes to shoot next bullet
var current_cooldown:float = shoot_cooldown * 2 # Main purpose of this is to give time for anim "ready gun" to play
var damage:float = 15 # How much damage each bullet does
